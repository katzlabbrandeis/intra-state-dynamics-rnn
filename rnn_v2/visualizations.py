...
# ----------------------------------------------------------------
# LOO diagnostics-- the whole leave-one-out business lives!
# ----------------------------------------------------------------
def plot_loo_diagnostics(info_criteria, dataset_name, taste_ind, plots_dir):
    ...
    6-panel figure:
        1. Per-trial held-out log-likelihood (bar + mean/SEM)
        2. Per-trial final training loss (bar + mean/SEM)
        3. All fold loss curves overlaid (overfitting diagnostic)
        4. LL vs final train loss scatter (outlier detection)
        5. Distribution of fold convergence steps (histogram)
        6. Summary stats text box

    Args:
        info_criteria: dict from loo_then_train with per_trial_ll,
                       per_trial_train_loss, per_fold_loss_history, etc.
        dataset_name: str
        taste_ind: int
        plots_dir: str
    """
    per_ll = np.array(info_criteria['per_trial_ll'])
    ...
    # --- Panel 1: Per-trial held-out LL ---
    ax = axes[0, 0]
    colors = ['salmon' if v < (np.mean(per_ll) - 2 * np.std(per_ll)) else 'steelblue'
              for v in per_ll]
    ax.bar(x, per_ll, color=colors)
    ax.axhline(np.mean(per_ll), color='k', linestyle='-', linewidth=1.5,
               label=f'Mean: {np.mean(per_ll):.1f}')
    ax.axhline(np.mean(per_ll) + np.std(per_ll), color='k', linestyle='--',
               alpha=0.5, label=f'$\pm$1 SD: {np.std(per_ll):.1f}')
    ax.axhline(np.mean(per_ll) - np.std(per_ll), color='k', linestyle='--',
               alpha=0.5)
    sem = np.std(per_ll) / np.sqrt(n_trials)
    ax.axhspan(np.mean(per_ll) - sem, np.mean(per_ll) + sem,
               color='gold', alpha=0.3, label=f'SEM: {sem:.1f}')
    ax.set_xlabel('Held-out Trial')
    ax.set_ylabel('Log-Likelihood')
    ax.set_title('Per-Trial Held-Out LL')
    ax.legend(fontsize=8)

    # --- Panel 2: Per-trial final training loss ---
    ax = axes[0, 1]
    ax.bar(x, per_loss, color='mediumpurple')
    ax.axhline(np.mean(per_loss), color='k', linestyle='-', linewidth=1.5,
               label=f'Mean: {np.mean(per_loss):.4f}')
    sem_loss = np.std(per_loss) / np.sqrt(n_trials)
    ax.axhspan(np.mean(per_loss) - sem_loss, np.mean(per_loss) + sem_loss,
               color='gold', alpha=0.3, label=f'SEM: {sem_loss:.4f}')
    ...
    if fold_histories:
        ...
        ax.plot(steps, mean_curve, color='darkblue', linewidth=2, label='Mean')
        ax.fill_between(steps, mean_curve - std_curve, mean_curve + std_curve,
                        color='steelblue', alpha=0.2, label='$\pm$1 SD')
        ax.legend(fontsize=8)
    ax.set_xlabel('Training Step')
    ax.set_ylabel('Loss')
    ax.set_title('Fold Loss Curves (overfitting check)')

    # --- Panel 4: LL vs final train loss scatter ---
    ax = axes[1, 0]
    ax.scatter(per_loss, per_ll, c='steelblue', edgecolors='k', s=50, alpha=0.7)
    worst_idx = np.argmin(per_ll)
    ax.annotate(f'Trial {worst_idx}',
                xy=(per_loss[worst_idx], per_ll[worst_idx]),
                fontsize=8, color='red',
                xytext=(5, -10), textcoords='offset points')
    ax.set_xlabel('Final Training Loss')
    ax.set_ylabel('Held-Out Log-Likelihood')
    ax.set_title('Train Loss vs Held-Out LL')
    if len(per_loss) > 2:
        corr = np.corrcoef(per_loss, per_ll)[0, 1]
    ...
    stats_lines = [
        ...
        f"Mean LL/trial:      {np.mean(per_ll):.2f}",
        f"SD LL:              {np.std(per_ll):.2f}",
        f"SEM LL:             {sem:.2f}",
        f"Min LL (trial {worst_idx}):  {per_ll[worst_idx]:.2f}",
        f"Max LL (trial {np.argmax(per_ll)}):  {np.max(per_ll):.2f}",
        f"{'=' * 35}",
        f"AIC:                {info_criteria['aic']:.2f}",
        f"BIC:                {info_criteria['bic']:.2f}",
        f"{'=' * 35}",
        f"Mean train loss:    {np.mean(per_loss):.4f}",
        f"SD train loss:      {np.std(per_loss):.4f}",
    ]
    if fold_steps:
        stats_lines += [
    ...
    fig.suptitle(f'LOO Diagnostics -- {dataset_name}, Taste {taste_ind}',
                 fontsize=14, y=1.02)
    fig.tight_layout()
    fig.savefig(
