...
def orchestrate_h5_input(cfg: PreprocConfig, *, force: bool = False, dry_run: bool = False,
    ...
    if force or not _has_ext(paths.info_dir, ".info"):
        if dry_run:
            print(f"[DRY] would copy .info from {cfg.h5_root} -> {paths.info_dir}")
        else:
            infos = find_copy_h5info(cfg.h5_root, paths.info_dir)
    ...
    if dry_run:
        print(f"[DRY] would scan {paths.npz_dir} and unpickle using RAW PKL at {cfg.pkl_root}")
        return
    ...
    for tup in extract_from_npz(paths.npz_dir):
        ...
        try:
            extracted = unpickle_changepoints(cfg.pkl_root, [(spike_array, dataset_num, index, key)])
    ...
    print(f"[C] processed changepoints for {npz_count} dataset(s) from RAW PKL at {cfg.pkl_root}")
